...
const createNewUser = asyncHandler(async (req, res) => {
    ...
    // confirm data
    if (!username || !password || email || phone || !Array.isArray(roles) || !roles.length) {
        return res.status(400).json({message: 'All fields are required!'})
    }

    // check for duplicates
    const duplicate = await User.findOne({username}).lean().exec(); // lean() returns a plain JS object instead of a mongoose document
    ...
    const userObject = {username, "password": hashedPwd, email, phone, roles};
...
const updateUser = asyncHandler(async (req, res) => {
    const {id, username, password, email, phone, roles, active} = req.body
    ...
    const user = await User.findById(id).exec(); // mongoose document
...
//use shift + alt + down arrow to duplicate the block of code
