...
router
  .route('/authenticate')
  .post(userController.verifyUser, (req, res) => res.end()) // authenticate user
router
  .route('/login')
  .post(rateLimiter, userController.verifyUser, userController.login) // login in app

/** GET Methods */
router
  .route('/getAllUsers')
  .get(userController.verifyUser, userController.getAllUsers) // get all users
router
  .route('/generateOTP')
  .get(userController.verifyUser, localVariables, userController.generateOTP) // generate random OTP
router
  .route('/verifyOTP')
  .get(userController.verifyUser, userController.verifyOTP) // verify generated OTP
...
router
  .route('/resetPassword')
  .put(userController.verifyUser, userController.resetPassword) // use to reset password
